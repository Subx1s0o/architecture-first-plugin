...
4. RICH PREVIEW before yes/no. Sections: Header / Architecture before→after (Mermaid flowchart LR with BEFORE+AFTER subgraphs, LoC on nodes, callers, delegation arrows) / Files touched (clickable repo-root-relative markdown links) / What moves / Callers that continue to work / Tests (existing pins + new + gaps) / Risks + mitigation / Alternatives / Destination. Ask: `yes | no | show more | tweak: <what>`. On tweak re-render.
